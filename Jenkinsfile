pipeline {
	agent any
	environment {
		DOCKERHUB_CREDENTIALS = credentials('olehepam')
	}
	...
	stages {
		...
		stage('Push to DockerHub') {
			steps {
				sh 'echo $DOCKERHUB_CREDENTIALS_PSW | sudo docker login -u $DOCKERHUB_CREDENTIALS_USR --password-stdin'
				sh 'sudo docker push $DOCKERHUB_CREDENTIALS_USR/olehepam:$GIT_BRANCH-$BUILD_NUMBER'
				sh 'sudo docker rmi -f $(sudo docker images -q $DOCKERHUB_CREDENTIALS_USR/olehepam:$GIT_BRANCH-$BUILD_NUMBER)'
			}
    		}
